          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm font-semibold text-blue-900">
                已选中 {{ selectedApiKeys.length }} 个 API Key
              </p>
              <p class="text-xs text-blue-700">可以进行批量操作</p>
            </div>
          </div>
          <div class="flex gap-2">
            <button
              class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-all hover:border-gray-400 hover:bg-gray-50"
            ...
            <button
              class="flex items-center gap-2 rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white transition-all hover:bg-red-700"
...
const isAllSelected = computed(() => {
  return (
    paginatedApiKeys.value.length > 0 &&
    paginatedApiKeys.value.every((key) => selectedApiKeys.value.includes(key.id))
  )
})

const isPartialSelected = computed(() => {
  const selected = paginatedApiKeys.value.filter((key) => selectedApiKeys.value.includes(key.id))
  return selected.length > 0 && selected.length < paginatedApiKeys.value.length
})
...
const loadApiKeys = async () => {
  ...
  try {
    ...
    if (data.success) {
      ...
      // 清理无效的选择项（已删除的API Keys）
      const validKeyIds = apiKeys.value.map((key) => key.id)
      selectedApiKeys.value = selectedApiKeys.value.filter((id) => validKeyIds.includes(id))
...
const toggleSelectAll = () => {
  if (isAllSelected.value) {
    // 如果当前页全部被选中，则取消全部选择
    selectedApiKeys.value = selectedApiKeys.value.filter(
      (id) => !paginatedApiKeys.value.some((key) => key.id === id)
    )
  } else {
    // 选中当前页的所有项目
    const currentPageIds = paginatedApiKeys.value.map((key) => key.id)
    const newSelected = [...new Set([...selectedApiKeys.value, ...currentPageIds])]
    selectedApiKeys.value = newSelected
  }
}

// 切换单项选择
const toggleSelectKey = (keyId) => {
  if (selectedApiKeys.value.includes(keyId)) {
    selectedApiKeys.value = selectedApiKeys.value.filter((id) => id !== keyId)
  } else {
    selectedApiKeys.value.push(keyId)
  }
}
...
const batchDeleteApiKeys = async () => {
  if (selectedApiKeys.value.length === 0) {
    showToast('请选择要删除的 API Keys', 'warning')
    return
  }

  const selectedKeys = apiKeys.value.filter((key) => selectedApiKeys.value.includes(key.id))
  const keyNames = selectedKeys.map((key) => key.name).join('、')

  let confirmed = false
  if (window.showConfirm) {
    confirmed = await window.showConfirm(
      '批量删除 API Keys',
      `确定要删除以下 ${selectedApiKeys.value.length} 个 API Keys 吗？\n\n${keyNames}\n\n此操作不可恢复。`,
      '确定删除',
      '取消'
    )
  } else {
  ...
  try {
    const data = await apiClient.delete('/admin/api-keys/batch', {
      data: { keyIds: selectedApiKeys.value }
    })
    ...
    if (data.success) {
      ...
      if (failedCount === 0) {
        ...
        showToast(`删除完成：成功 ${successCount} 个，失败 ${failedCount} 个`, 'warning')
...
watch([currentPage, pageSize], () => {
  ...
  const currentPageIds = paginatedApiKeys.value.map((key) => key.id)
  selectedApiKeys.value = selectedApiKeys.value.filter(
    (id) => apiKeys.value.some((key) => key.id === id) // 保留仍然存在的项目
  )
})
